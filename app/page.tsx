...
export default function YouTubeMusicApp() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-card border-b border-border sticky top-0 z-40 backdrop-blur-sm bg-card/95">
        <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-foreground">{"Liminis Play"}</h1>
          </div>

          <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
            <div className="hidden sm:flex items-center gap-2 text-muted-foreground">
              <Music className="w-4 h-4" />
              <span className="text-sm lg:text-base truncate max-w-24 sm:max-w-32 lg:max-w-none">{user.name}</span>
            </div>
            <Button
              variant="ghost"
              size="sm"
              onClick={handleLogout}
              className="text-muted-foreground hover:text-foreground text-sm min-h-[44px] px-3 sm:px-4"
            >
      ...
      {/* Main Content */}
      <main className="container mx-auto px-4 sm:px-6 py-4 sm:py-6 lg:py-8 pb-32 sm:pb-36">
        <Tabs defaultValue="search" className="space-y-4 sm:space-y-6">
          <TabsList className="bg-muted w-full sm:w-auto grid grid-cols-2 sm:flex h-12 sm:h-10">
            <TabsTrigger value="search" className="flex items-center gap-2 text-sm sm:text-base min-h-[44px]">
              <Search className="w-4 h-4" />
              <span>Search</span>
            </TabsTrigger>
            <TabsTrigger value="favorites" className="flex items-center gap-2 text-sm sm:text-base min-h-[44px]">
              <Heart className="w-4 h-4" />
              <span>Favorites</span>
              {favorites.length > 0 && (
                <span className="ml-1 px-2 py-1 bg-primary text-primary-foreground text-xs rounded-full min-w-[20px] text-center">
                  {favorites.length}
                </span>
              )}
            </TabsTrigger>
