    ...
    <Card className="fixed bottom-0 left-0 right-0 z-50 rounded-none border-t shadow-lg">
      <CardContent className="p-2 sm:p-4">
        <div className="flex flex-col space-y-2 sm:space-y-0 sm:flex-row sm:items-center sm:gap-4">
          {/* Video Info */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 sm:flex-none sm:w-48 lg:w-64">
            <img
            ...
            <div className="min-w-0 flex-1">
              <h4 className="font-medium text-xs sm:text-sm truncate" title={video.title}>
                {video.title}
              </h4>
          ...
          <div className="flex flex-col items-center gap-1 sm:gap-2 flex-1">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <Button
                ...
                className={`p-1 sm:p-2 ${isShuffle ? "text-primary" : ""}`}
                title="Shuffle"
              >
                <Shuffle className="w-3 h-3 sm:w-4 sm:h-4" />
              </Button>
              <Button size="sm" variant="ghost" onClick={handlePrevious} className="p-1 sm:p-2" title="Previous">
                <SkipBack className="w-3 h-3 sm:w-4 sm:h-4" />
              </Button>
              <Button size="sm" onClick={handlePlayPause} className="p-2 sm:p-3" title={isPlaying ? "Pause" : "Play"}>
                {isPlaying ? <Pause className="w-4 h-4 sm:w-5 sm:h-5" /> : <Play className="w-4 h-4 sm:w-5 sm:h-5" />}
              </Button>
              <Button size="sm" variant="ghost" onClick={handleNext} className="p-1 sm:p-2" title="Next">
              ...
              <Button
                ...
                className={`p-1 sm:p-2 ${isRepeat ? "text-primary" : ""}`}
                title="Repeat"
              >
            ...
            {/* Progress Bar */}
            <div className="flex items-center gap-1 sm:gap-2 w-full max-w-xs sm:max-w-md">
              <span className="text-xs text-muted-foreground w-8 sm:w-10 text-center">{formatTime(currentTime)}</span>
              <Slider value={[currentTime]} max={duration} step={1} onValueChange={handleSeek} className="flex-1" />
              <span className="text-xs text-muted-foreground w-8 sm:w-10 text-center">{formatTime(duration)}</span>
            </div>
          </div>
